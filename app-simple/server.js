...
// ─── PostgreSQL ──────────────────────────────────────────────────────────────
// DATABASE_URL is injected by `bella run --` at container startup.
...
app.get('/products', async (_req, res) => {
  try {
    ...
    res.json({ ok: true, count: result.rowCount, products: result.rows });
  } catch (err) {
...
app.get('/secrets-demo', (_req, res) => {
  const BELLA_VARS = ['BELLA_BAXTER_URL', 'BELLA_BAXTER_API_KEY', 'BELLA_ENV', 'BELLA_PROJECT'];
  const APP_VARS   = ['DATABASE_URL', 'PORT', 'NODE_ENV'];
...
async function main() {
  if (!process.env.DATABASE_URL) {
    console.warn('⚠️  DATABASE_URL is not set. Make sure "bella run --" is the ENTRYPOINT.');
  }
